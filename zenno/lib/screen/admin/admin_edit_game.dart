...
class _AdminEditGameScreenState extends ConsumerState<AdminEditGameScreen> {
  ...
  Future<void> _pickPublishedDate() async {
    ...
    final existing = _publishedDateController.text.trim();
    if (existing.isNotEmpty) {
      try {
        ...
        if (parts.length == 3) {
          ...
          if (month > 0) initial = DateTime(year, month, day);
        }
      } catch (_) {}
    }
                ...
                child: ref.watch(gameByIdProvider(widget.gameId)).when(
                  data: (game) {
                    if (game != null && !_loadedGame) {
                      ...
                      _discount = ((game['discount'] as num?) ?? 0).toInt().clamp(0, 80);
                      _currency = (game['currency'] ?? 'USD').toString();
                    }
